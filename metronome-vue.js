...
class Metronome{
    constructor(config) {
        ...
        this.isWorking = ref(false) // 是否在运行
        this.activeSubdivision = 0 // 当前细分索引

        this.beatCount = ref(config.beatCount || 4) // 总节拍数
        this.beatIndex =  1 // 当前拍数
        this.stressFirstBeat = config.stressFirstBeat || true // 第一拍重音
        this.currentBpm = ref(config.bpm || 100) // bpm参数
        this.subdivisionPattern = ref(config.subdivisionPattern || [1]) // 细分
    ...
    increaseActiveSubdivision(){
        if (this.activeSubdivision + 1 >= this.subdivisionPattern.value.length) {
    ...
    beats(){
        const _this = this
        // 计算播放间隔
        const speed = 60 / this.currentBpm.value * 1000 / this.subdivisionPattern.value.length;
        const isLastBeat = this.beatIndex >= this.beatCount.value; // 相对或者超出都是最后拍

        // 细分处理 - 如果设置了细分，并且不是第一拍
        if (this.subdivisionPattern.value.length > 1 && this.activeSubdivision > 0) {
            if (this.subdivisionPattern.value[this.activeSubdivision] == 1) {
    ...
    static getInstance(config) {
        if (!this.instance) {
            this.instance = new Metronome(config);
        }
        return this.instance;
    }

    set bpm(v){
        this.currentBpm.value = v
    }

    set beat(v){
        this.beatCount.value = v
    }

    set subdivision(v){
        this.subdivisionPattern.value = v
    }
}
